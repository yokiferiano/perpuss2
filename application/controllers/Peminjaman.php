...
class Peminjaman extends CI_Controller{
	...
	public function tambah_pinjam($id){
		if($this->session->userdata('status') != "login"){
			...
			$o =
			$this->M_perpus->edit_data(array('id_buku'=>$id),'transaksi')->num_rows();
				if($o>0) {
	...
	public function lihat_keranjang(){
		...
		$data['peminjaman'] = $this->db->query("SELECT * FROM transaksi t,buku b,anggota a WHERE b.id_buku=t.id_buku and a.id_anggota=t.id_anggota anda.id_anggota=$where")->result();
